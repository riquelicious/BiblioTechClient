...
const DisplayBorrower = () => {
  ...
  const handleDelete = () => {
    if (selectedBooks.length === 0)
      ...
      const DisplayBorrower = async () => {
        try {
          const response = await window.electronAPI.DisplayBorrower(
            selectedBooks
          );
  ...
  const fetchEntries = async (page, filter, search) => {
    try {
      const response = await window.electronAPI.fetchBorrower(
        page,
        filter,
        search
      );
      ...
      if (response?.data?.Borrower) {
        setEntries(response.data?.Borrower);
...
function TableHeader(props) {
  return (
    <div className="borrower-table-heading">
      <div className="checkbox-column-new">
        <input
          type="checkbox"
          onChange={props.onChange}
