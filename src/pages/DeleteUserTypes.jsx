...
const DeleteUserTypes = () => {
  ...
  const fetchEntries = async (page, filter, search) => {
    try {
      const response = await window.electronAPI.fetchUserTypes(
        page,
        filter,
        search
      );
      ...
      if (response?.data?.UserTypes) {
        setEntries(response.data?.UserTypes);
...
function TableHeader(props) {
  return (
    <div className="UserTypes-table-heading">
      <div className="checkbox-column-new">
        <input
          type="checkbox"
          onChange={props.onChange}
