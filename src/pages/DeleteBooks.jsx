  ...
  const handleDelete = () => {
    if (selectedBooks.length === 0)
      ...
      const deleteBooks = async () => {
        try {
          const response = await window.electronAPI.deleteBooks(selectedBooks);
  ...
  const fetchEntries = async (page, filter, search) => {
    try {
      ...
      if (response?.data?.Books) {
        setEntries(response.data?.Books);
...
function TableHeader(props) {
  return (
    <div className="books-table-heading">
      <div className="checkbox-column-new">
        <input
          type="checkbox"
          onChange={props.onChange}
