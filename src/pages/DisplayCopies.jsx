...
const DisplayCopies = () => {
  ...
  const handleDelete = () => {
    if (selectedBooks.length === 0)
      ...
      const DisplayCopies = async () => {
        try {
          const response = await window.electronAPI.DisplayCopies(
            selectedBooks
          );
  ...
  const fetchEntries = async (page, filter, search) => {
    try {
      const response = await window.electronAPI.fetchCopies(
        page,
        filter,
        search
      );

      if (response?.data?.Copies) {
        setEntries(response.data?.Copies);
...
function TableHeader(props) {
  return (
    <div className="copies-table-heading">
      <div className="checkbox-column-new">
        <input
          type="checkbox"
          onChange={props.onChange}
